...
public class CreateChargeAccum {	
	public static void main(String[] args) throws IOException, SQLException {
		System.out.println("CreateChargeAccum");
        ...
        System.out.println("Table ACCUM ="+stCYCLE_CONTROL.get(0).getACCUM_BUCKET());
        ...
        for(int i=0;i<idACCUM_USAGE_XX.size();i++){
        	//System.out.println(stACCUM_USAGE_XX.get(i).getACCOUNT_ID());
        	// map CUSTOMER_ACCOUNT
        	
        	int indexCUSTOMER_ACCOUNT=idCUSTOMER_ACCOUNT.indexOf(idACCUM_USAGE_XX.get(i));
        	System.out.println(idACCUM_USAGE_XX.get(i));
        	if(indexCUSTOMER_ACCOUNT>=0){
        		stCHARGE.add(new CHARGE(
            			stACCUM_USAGE_XX.get(i).getACCOUNT_ID(), 
            			null, 
            			null, 
            			sysDateTime, 
            			null, 
            			...
            			CYCLE_YEAR, 
            			CYCLE_MONTH, 
            			stACCUM_USAGE_XX.get(i).getPRODUCT_TYPE(), 
            			stACCUM_USAGE_XX.get(i).getPRODUCT_NO(), 
            			logical_date, 
            			"DBT", 
            			"N", 
            			stACCUM_USAGE_XX.get(i).getPRICE_PLAN(), 
            			stACCUM_USAGE_XX.get(i).getFEATURE_CODE(), 
            			"U", 
            			null, 
            			null, 
            			null, 
            			null, 
            			null, 
            			stACCUM_USAGE_XX.get(i).getCHARGE_AMT(), 
            			stCUSTOMER_ACCOUNT.get(indexCUSTOMER_ACCOUNT).getTAX_CODE(), 
            			null, 
            			stACCUM_USAGE_XX.get(i).getTOTAL_NUMBER_OF_CALLS()
            			));
