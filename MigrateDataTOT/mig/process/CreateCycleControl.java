...
public class CreateCycleControl {
	// CreateCycleControl_sh /home/appadm/var/tea/MigData/CYCLE_CONTROL/input/ 01
	public static void main(String[] args) throws Exception {
		String pathFileInput = args[0];
		String pidFile = args[1];

		String runOutput = runCommand
				.executeCommand("touch " + Constants.cycleControlPid + "CreateCycleControl_" + pidFile + ".pid");
		System.out.println(runOutput);

		Path pathPid = Paths.get(Constants.cycleControlPid + "CreateCycleControl_" + pidFile + ".pid");
		...
		try {
			...
			String sql = ProcessSQL.readSql(Constants.cycleControlXLS);
			int count = Sql8i.wirteValueSql(conn8i, sql, pathFileInput + "Cycle_Control.txt");

			sql = ProcessSQL.readSqlInsert(Constants.cycleControlXLS);
			recordInsert = Sql11g.insertFromFile(conn11g, sql, pathFileInput + "Cycle_Control.txt");
			...
			if (staturProcess == 0) {
				runOutput = runCommand.executeCommand(
						"mv " + pathFileInput + "Cycle_Control.txt" + " " + Constants.cycleControlBackUp);
				System.out.println(runOutput);
			}
