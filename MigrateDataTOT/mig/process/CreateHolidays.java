...
public class CreateHolidays {
	// CreateHolidays_sh /home/appadm/var/tea/MigData/HOLIDAYS/input/ 01
	public static void main(String[] args) throws Exception {
		String pathFileInput = args[0];
		String pidFile = args[1];

		String runOutput = runCommand
				.executeCommand("touch " + Constants.holidaysPid + "CreateHolidays_" + pidFile + ".pid");
		System.out.println(runOutput);

		Path pathPid = Paths.get(Constants.holidaysPid + "CreateHolidays_" + pidFile + ".pid");
		...
		try {
			...
			String sql = ProcessSQL.readSql(Constants.holidaysXLS);
			int  count = Sql8i.wirteValueSql(conn8i, sql, pathFileInput + "Holidays.txt");

			sql = ProcessSQL.readSqlInsert(Constants.holidaysXLS);
			recordInsert = Sql11g.insertFromFile(conn11g, sql, pathFileInput + "Holidays.txt");
			...
			if (staturProcess == 0) {
				runOutput = runCommand.executeCommand(
						"mv " + pathFileInput + "Holidays.txt" + " " + Constants.holidaysBackUp);
				System.out.println(runOutput);
			}
