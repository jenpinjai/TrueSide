...
public class CreateRcRate {
	// CreateRcRate_sh /home/appadm/var/tea/MigData/RC_RATE/input/ 01
	public static void main(String[] args) throws Exception {
		String pathFileInput = args[0];
		String pidFile = args[1];

		String runOutput = runCommand.executeCommand("touch " + Constants.rcRatePid + "CreateRcRate_" + pidFile + ".pid");
		System.out.println(runOutput);

		Path pathPid = Paths.get(Constants.rcRatePid + "CreateRcRate_" + pidFile + ".pid");
		...
		try {
			...
			String sql = ProcessSQL.readSql(Constants.rcRateXLS);
			int count = Sql8i.wirteValueSql(conn8i, sql, pathFileInput + "RcRate.txt");

			sql = ProcessSQL.readSqlInsert(Constants.rcRateXLS);
			recordInsert = Sql11g.insertFromFile(conn11g, sql, pathFileInput + "RcRate.txt");
			...
			if (staturProcess == 0) {
				runOutput = runCommand
						.executeCommand("mv " + pathFileInput + "RcRate.txt" + " " + Constants.rcRateBackUp);
				System.out.println(runOutput);
			}
