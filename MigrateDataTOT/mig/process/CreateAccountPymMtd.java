...
public class CreateAccountPymMtd {
	// CreateAccountPymMtd_sh /home/appadm/var/tea/MigData/ACCOUNT_PYM_MTD/input/process01/ 01
	public static void main(String[] args) throws Exception {
		String pathFileInput=args[0];		
		String pidFile=args[1];
		
		String runOutput = runCommand.executeCommand("touch "+Constants.accountPymMtdPid+"CreateServiceFeature_"+pidFile+".pid");
		System.out.println(runOutput);

		Path pathPid = Paths.get(Constants.accountPymMtdPid+"CreateServiceFeature_"+pidFile+".pid");
			...
			for(int i=0;i<listFile.size();i++){
				...
				try (BufferedReader reader = Files.newBufferedReader(Paths.get(pathFileInput+listFile.get(i)), Constants.UTF_8);){
					...
					while((accountIdList = reader.readLine()) != null){
						...
						String sql = ProcessSQL.readSql(Constants.accountPymMtdXLS);
						sql += " ( " + accountIdList + ") ";
						Sql8i.load(conn8i, sql, lst);
						
						sql = ProcessSQL.readSqlInsert(Constants.accountPymMtdXLS);
						...
						if(staturProcess==0){
							runOutput = runCommand.executeCommand("mv "+pathFileInput+listFile.get(i)+" "+Constants.accountPymMtdBackUp);
							System.out.println(runOutput);
						}
