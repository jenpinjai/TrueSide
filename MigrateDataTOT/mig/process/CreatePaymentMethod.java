...
public class CreatePaymentMethod {
	// CreatePaymentMethod_sh /home/appadm/var/tea/MigData/PAYMENT_METHOD/input/ 01
	public static void main(String[] args) throws Exception {
		String pathFileInput = args[0];
		String pidFile = args[1];

		String runOutput = runCommand
				.executeCommand("touch " + Constants.paymentMethodPid + "CreatePaymentMethod_" + pidFile + ".pid");
		System.out.println(runOutput);

		Path pathPid = Paths.get(Constants.paymentMethodPid + "CreatePaymentMethod_" + pidFile + ".pid");
		...
		try {
			...
			String sql = ProcessSQL.readSql(Constants.paymentMethodXLS);
			int  count = Sql8i.wirteValueSql(conn8i, sql, pathFileInput + "PaymentMethod.txt");

			sql = ProcessSQL.readSqlInsert(Constants.paymentMethodXLS);
			recordInsert = Sql11g.insertFromFile(conn11g, sql, pathFileInput + "PaymentMethod.txt");
			...
			if (staturProcess == 0) {
				runOutput = runCommand.executeCommand(
						"mv " + pathFileInput + "PaymentMethod.txt" + " " + Constants.paymentMethodBackUp);
				System.out.println(runOutput);
			}
